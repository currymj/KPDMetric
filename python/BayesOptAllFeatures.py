...
if __name__ == '__main__':
    for fi in range(13, 14):  # feature index - between 0 and 14
        print("Starting Bayesian Optimization for feature: " +
              complete_range[fi])
        ...
        if (not(np.array_equal(XL[fi], np.empty([0, 5])) or
                np.array_equal(XL[fi], np.empty([0, 6])))):
        ...
        myBopt.run_optimization(
            max_iter=100,
            eps=.1,
            evaluations_file=output_dir + "E"+str(fi)+".txt",
            models_file=output_dir+"M"+str(fi)+".txt")
